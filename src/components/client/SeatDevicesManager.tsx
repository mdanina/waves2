...
import { SeatEmailSetup } from './SeatEmailSetup';
...
// Иконки устройств
function DeviceIcon({ type }: { type: SeatDevice['device_type'] }) {
  switch (type) {
    case 'mobile':
...
interface SeatDevicesManagerProps {
  seatId: string;
  profileName?: string;
...
export function SeatDevicesManager({
  ...
  const {
    ...
    cancelUnbind,
    _mockCode,
  } = useSeatDevices({ seatId });

  // Состояние UI
  const [selectedDevice, setSelectedDevice] = useState<SeatDevice | null>(null);
  ...
  const handleRequestUnbind = async (device: SeatDevice) => {
    setError(null);
  ...
  // Отмена отвязки
  const handleCancelUnbind = async (device: SeatDevice) => {
  ...
  // Если email не привязан — показываем форму настройки
  if (!hasEmail) {
    return (
      <div className={cn('space-y-4', className)}>
        <h3 className="text-lg font-medium">
          Настройка приложения {profileName && `для ${profileName}`}
        </h3>
        <SeatEmailSetup
          seatId={seatId}
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium">
          Устройства {profileName && `(${profileName})`}
        </h3>
      ...
      {binding?.email && (
        <div className="flex items-center gap-3 p-3 bg-cloud/50 rounded-xl">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm text-muted-foreground">Email для приложения</div>
      ...
      <p className="text-sm text-muted-foreground">
        Устройства привязываются автоматически при входе в мобильное приложение по email.
