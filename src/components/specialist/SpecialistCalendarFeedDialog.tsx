...
import {
  generateSpecialistCalendarFeedToken,
  revokeSpecialistCalendarFeedToken,
  getExistingSpecialistCalendarToken,
  buildFeedUrl,
} from '@/lib/calendar-feed';

interface SpecialistCalendarFeedDialogProps {
  specialistId: string;
  trigger?: React.ReactNode;
}

export function SpecialistCalendarFeedDialog({ specialistId, trigger }: SpecialistCalendarFeedDialogProps) {
  ...
  const loadExistingToken = async () => {
    setLoading(true);
    try {
      const token = await getExistingSpecialistCalendarToken(specialistId);
      if (token) {
  ...
  const handleGenerateLink = async () => {
    setLoading(true);
    try {
      const result = await generateSpecialistCalendarFeedToken();
  ...
  const handleRevokeLink = async () => {
    setLoading(true);
    try {
      await revokeSpecialistCalendarFeedToken();
      setFeedUrl(null);
      toast({
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Добавьте расписание консультаций в свой календарь.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <p className="text-xs text-muted-foreground">
            Календарь будет автоматически обновляться при изменении расписания.
            В событиях будут указаны имена клиентов и ссылки на видеоконсультации.
          </p>
        </div>
      </DialogContent>
    </Dialog>
